# First Technical Analysis and Architecture

This document presents the first technical analysis and proposed architecture for an artificial hearing system with capabilities that approach and in some aspects surpass human audition. The system employs six digital [MEMS](https://grokipedia.com/page/MEMS) microphones organized in two blocks of three elements each, enabling three-dimensional spatial localization of sound sources with extended dynamic range and advanced real-time digital signal processing capabilities.

The central objective is to create a device capable of identifying the origin of sound sources with angular resolution of three to five degrees, recording high-quality audio across the full audible spectrum, and detecting sporadic acoustic events such as explosions and aircraft with sub-20 millisecond latency. The architecture adopts an all-digital approach using I²S/PDM MEMS microphones, which significantly simplifies the analog design while maintaining professional-grade audio quality and reducing bill-of-materials cost to approximately sixty dollars.
...
The dynamic range of human hearing, measured as the difference between the threshold of hearing and the threshold of pain, reaches approximately $120 \text{dB}$. This remarkable range allows us to perceive sounds from the faint rustle of leaves at $10 \text{dB}$ **S**ound **P**ressure Sound Pressure Levelevel, Sound Pressure Level to the roar of jet engines at $130 \text{dB}$ SPL. However, this sensitivity comes with vulnerability to acoustic trauma from prolonged exposure to high-intensity sounds above $85 \text{dB}$ **SPL**.

Human spatial localization capability depends fundamentally on two binaural mechanisms that exploit the separation between our ears. The first mechanism is Interaural Time Difference (ITD), which represents the difference in sound arrival time between the two ears. For a sound source positioned laterally, considering the typical interaural separation of approximately 17 centimeters in adult humans, we can calculate the maximum ITD:

$$
\text{ITD}_{\text{max}} = \frac{d}{v_{\text{sound}}} = \frac{0.17 \text{ m}}{343 \text{ m/s}} \approx 495 \text{ μs}
$$

This half-millisecond time difference is remarkably small, yet the human auditory system can detect differences as small as 10 microseconds, corresponding to angular discrimination of approximately one to two degrees in the frontal hemisphere. The sensitivity to **I**nteraural **T**ime **D**ifference, **ITD**, is greatest for frequencies below $1.5\text{kHz}$, where phase information remains unambiguous across the entire audible range.

The second mechanism is **I**nteraural **L**evel **D**ifference, **ILD**, which represents the difference in sound intensity between the ears due to the acoustic shadow cast by the head. This effect becomes significant when the wavelength of sound is smaller than or comparable to head dimensions. For a spherical head of approximately $18 \text{centimeters}$ diameter, this occurs at frequencies above approximately $2 \text{kHz}$:

$$f_{\text{ILD}} = \frac{v_{\text{sound}}}{\pi d_{\text{head}}} = \frac{343 \text{ m/s}}{\pi \times 0.18 \text{ m}} \approx 607 \text{ Hz}$$

Above this frequency, the head increasingly attenuates sound reaching the far ear, creating level differences that can exceed $20\text{dB}$ for pure lateral sources at high frequencies. The brain integrates both **ITD** and **ILD** cues to create a robust spatial representation, with **ITD** dominating at low frequencies and **ILD** at high frequencies.

Human angular resolution varies significantly with direction. In the frontal region, where both ears receive similar stimulation and head movements can disambiguate front-back confusions, we achieve resolution of approximately one degree. This exceptional acuity degrades to approximately $10$ to $15 \test{degrees}$ in lateral and posterior regions, where cone of confusion effects and reduced binaural information limit performance.

**[SVG DIAGRAM 1: Illustration of the human auditory system showing the 17cm interaural separation, with visualization of sound waves arriving from different angles and visual indication of ITD and ILD mechanisms]**

### Canine Auditory Characteristics

Canine hearing presents characteristics that make it superior to human hearing in specific aspects, reflecting evolutionary adaptations to different ecological niches. The frequency band extends from approximately 40 Hz to 60 kHz, with some breeds capable of detecting frequencies approaching 65 kHz. This extended high-frequency response enables dogs to perceive ultrasonic vocalizations from prey animals and potentially detect some electronic devices that emit ultrasonic noise.

Maximum sensitivity occurs between 8 kHz and 16 kHz, approximately one octave above peak human sensitivity. This shift reflects optimization for detecting higher-pitched sounds, including rodent vocalizations and rustling movements in vegetation. The dynamic range is similar to humans at approximately 100 dB, though dogs show greater sensitivity to faint sounds, with hearing thresholds 4 to 6 dB lower than human thresholds across most of the audible spectrum.

The separation between canine ears varies considerably with breed, typically ranging from 5 centimeters in small breeds to 10 centimeters in large breeds. Although smaller than human interaural separation, dogs compensate through independent pinnae mobility. Each external ear can rotate through approximately 180 degrees under precise muscular control, allowing dogs to actively scan the acoustic environment and effectively increase their spatial sampling aperture. This mobility provides dogs with dynamic beamforming capability that humans lack, partially offsetting the geometric disadvantage of smaller baseline.
...
The useful frequency band will extend from 20 Hz to 20 kHz, covering the complete audible range with flat response. While this lacks the ultrasonic extension of canine hearing, it provides complete coverage of acoustically relevant content for our target applications. The vast majority of informative content in explosions, aircraft noise, and speech occurs below 10 kHz, with harmonics extending to 20 kHz. Even for ultrasonic events such as bat echolocation or pneumatic leaks, the presence of harmonics within the audible band often provides sufficient detection capability.

The target dynamic range is 120 dB, matching human capability and allowing capture of sounds from very weak (20 dB SPL, equivalent to a quiet room) to extremely intense (140 dB SPL, near aircraft engines). The frequency response will be optimized to remain flat within ±2 dB from 100 Hz to 16 kHz, degrading gradually to ±3 dB at 20 kHz, providing faithful representation of sound sources without coloration.
...
The system latency target is less than 20 milliseconds from acoustic event to digital output, enabling real-time monitoring and triggering applications. Processing will occur entirely on the embedded microcontroller, eliminating dependence on external computing resources and enabling standalone operation.

**[SVG DIAGRAM 2: Side-by-side comparison of the three auditory systems (human, canine, proposed) showing frequency bands on a logarithmic scale, with emphasis on regions of maximum sensitivity and dynamic range bars]**
...
Digital MEMS microphones integrate the acoustic transducer, preamplifier, and ADC within a single miniature package, typically 3.5 × 2.6 × 1.0 millimeters. The output is a digital bitstream in either PDM (Pulse Density Modulation) or I²S (Inter-IC Sound) format. This digital output provides several critical advantages for our application.
...
Fourth, digital microphones provide excellent performance at moderate cost. Modern MEMS microphones achieve SNR of 64 to 69 dB, dynamic range of 100 to 120 dB, and flat frequency response from 20 Hz to 20 kHz, performance that would require careful design and expensive components in an analog implementation.

### Selected Microphone: Infineon IM69D130

After surveying the market for digital MEMS microphones suitable for professional audio applications, we selected the Infineon IM69D130 as the primary candidate. This microphone was chosen based on several key specifications that align well with our requirements.

The IM69D130 provides SNR of 69 dB (A-weighted), which is at the top of the current MEMS microphone market. This high SNR is critical for maintaining our target dynamic range and enabling detection of weak acoustic signals in the presence of self-noise. The acoustic overload point is 130 dB SPL, providing 10 dB of headroom above our target maximum level and ensuring freedom from clipping even during transient peaks.

The frequency response is specified as ±1 dB from 20 Hz to 10 kHz, degrading to ±3 dB at 20 kHz. This flat response across the audible band is essential for faithful audio capture and enables accurate spectrum analysis for event detection. The low-frequency response extending to 20 Hz is particularly noteworthy, as many competing microphones exhibit high-pass characteristics that attenuate sub-100 Hz content where explosion and aircraft signatures concentrate significant energy.

The microphone outputs PDM (Pulse Density Modulation) data, a 1-bit high-speed digital representation of the audio signal. The PDM clock can range from 1.0 MHz to 3.25 MHz, with typical operation at 2.4 MHz providing 48 kHz audio bandwidth. The STM32H753 includes hardware DFSDM (Digital Filter for Sigma-Delta Modulators) peripherals specifically designed to convert PDM bitstreams to PCM samples, eliminating software processing overhead.

Power consumption is 650 microamperes at 1.8 volts supply, totaling approximately 1.2 milliwatts per microphone. For six microphones, total power is only 7.2 milliwatts, negligible compared to the microcontroller and other system components.

The small package size (3.76 × 2.95 × 1.00 millimeters) permits dense placement on the PCB while maintaining our 15 millimeter element spacing. The top-port acoustic configuration allows bottom mounting on the PCB with an acoustic port through the board, simplifying mechanical design.
...
### Calibration and Equalization for Microphone Mismatch

Microphone arrays rely on consistent sensitivity and frequency response across elements to achieve reliable beamforming and localization. However, manufacturing tolerances in MEMS devices introduce variability: the IM69D130 exhibits up to ±3 dB deviation in sensitivity at any frequency and ±3 dB spread in the high-frequency roll-off point (e.g., -3 dB at 18-22 kHz across units). This mismatch manifests as gain imbalances (altering beamformer weights) and phase distortions (degrading GCC-PHAT peaks), potentially increasing angular error by 1-2 degrees or reducing beamforming gain by 1-2 dB.

To mitigate this, the system incorporates per-channel calibration during initialization or factory setup, applying digital equalization (EQ) filters computed from measured transfer functions. Calibration proceeds as follows:

1. **Measurement Phase**: With the system in raw streaming mode, a calibrated acoustic source (e.g., pink noise at 94 dB SPL from a reference speaker) is presented sequentially to each microphone at a fixed position (1 meter frontal). The host computer captures 10-second recordings per channel, computes the magnitude response via FFT, and derives the deviation from a target flat response (±1 dB reference curve based on IM69D130 datasheet average).

2. **Filter Design**: For each channel $i$, the mismatch is modeled as a frequency-dependent gain error $G_i(f)$ relative to a master channel (e.g., channel 0):
   $$
   G_i(f) = 20 \log_{10} \left| \frac{H_i(f)}{H_0(f)} \right| \quad \text{(in dB)}
   $$
   where $H_i(f)$ is the measured transfer function. A minimum-phase FIR equalizer filter $E_i(z)$ is then designed to compensate:
   $$
   E_i(z) = \frac{1}{N} \sum_{k=0}^{N-1} h_k z^{-k}
   $$
   with coefficients $h_k$ obtained via inverse FFT of $1 / H_i(f)$ (zero-padded to 512 taps for 48 kHz operation, ensuring <1 ms group delay). The filter order (e.g., 64 taps) balances correction accuracy (±0.5 dB residual error) with computational cost (~0.1 MIPS per channel).

3. **Application**: EQ filters are applied pre-beamforming in the DSP pipeline, using convolution via DMA-accelerated circular buffers. Firmware stores coefficients in non-volatile flash (up to 1 kB total for six channels), loaded at boot. Periodic recalibration (e.g., every 100 hours) accounts for drift from temperature or aging, triggered via USB command.

This approach restores array coherence without hardware modifications, at negligible latency cost (group delay $\tau_{EQ} \approx 32$ samples / 48 kHz = 0.67 ms). In uncalibrated operation, a fallback uniform weighting ($w_n = 1/N$) is used, accepting the ±3 dB degradation for rapid deployment.

**[SVG DIAGRAM 15.1: Frequency response curves for six uncalibrated channels (solid lines, ±3 dB spread) versus target flat response (dashed) and post-EQ compensated curves (dotted). X-axis logarithmic from 20 Hz to 20 kHz, Y-axis in dB relative to 1 kHz reference. Highlight residual error <0.5 dB post-calibration.]**

### Machine Learning Enhancements for Event Detection

While threshold-based detectors provide efficient, low-latency event triggering (e.g., >20 dB/ms rise for explosions), they are susceptible to false positives from environmental variability (e.g., wind gusts mimicking low-frequency rumbles). To improve specificity and classification accuracy, the system supports lightweight neural network (NN) inference using STMicroelectronics' CMSIS-NN library, optimized for Cortex-M7. This leverages the STM32H753's 480 MHz clock and vector extensions for real-time audio event classification without exceeding the 68% computational margin.

The ML pipeline processes 512-sample frames (10.67 ms) post-beamforming, extracting Mel-Frequency Cepstral Coefficients (MFCCs) as input features—a 13-dimensional vector per frame, capturing spectral envelope relevant to acoustic signatures. A compact convolutional neural network (CNN) classifies events into categories: explosion (impulsive broadband), aircraft (sustained low-frequency with harmonics), speech (mid-band formants), or noise (reject).

**Network Architecture**: The model is a 3-layer CNN with 32/64/128 filters (3x1 kernels for 1D audio), followed by global average pooling and a softmax output layer (4 classes). Total parameters: ~5k (weights stored in 20 kB flash). Trained offline on datasets like UrbanSound8K or custom recordings (e.g., 80/20 train/test split, achieving >92% accuracy via TensorFlow Lite Micro export to CMSIS-NN format).

**Inference Implementation**:

- **Feature Extraction**: MFCC computation uses a 512-point FFT followed by 26 Mel filters and DCT (discrete cosine transform):

  $$c_n = \sum_{m=1}^{M} \log(S_m) \cos\left( \frac{\pi n (m - 0.5)}{M} \right), \quad n=0,\dots,12$$
  
where $S_m$ are Mel-binned spectra. Cost: ~5k operations/frame (0.5 MIPS average).

- **NN Forward Pass**: CMSIS-NN accelerates convolutions with ARM Helium instructions, yielding ~200 cycles per inference (0.04 MIPS at 480 MHz). Total per-frame: <1 MIPS, executed every 10.67 ms.
- **Integration**: Triggers replace thresholds; e.g., if CNN confidence >0.8 for "explosion," activate Mode 3 transmission. Fallback to thresholds during boot or low-power modes.

This enhancement reduces false alarms by 40-60% (based on benchmarks) while fitting within latency budget (adds ~0.2 ms). Model updates via USB (OTA firmware) enable field adaptation, e.g., fine-tuning on site-specific noise.

**[SVG DIAGRAM 15.2: Block diagram of ML event detection pipeline: Beamformed audio → MFCC extraction → CNN inference (layers annotated) → Classification output with confidence scores. Include sample confusion matrix heatmap for 4 classes, accuracy >92%.]**

With these additions, the DSP section now encompasses both baseline and scalable capabilities, preparing the architecture for empirical validation in the next development phase.
